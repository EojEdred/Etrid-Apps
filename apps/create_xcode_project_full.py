#!/usr/bin/env python3
"""
Automated Xcode iOS App Project Creator - Full Version
Creates a complete Xcode project with all Swift files automatically discovered
"""
...
def discover_swift_files(app_path):
    """Recursively discover all .swift files in the app directory"""
    swift_files = []
...
def create_project_pbxproj(project_path, app_path, source_files):
...
def main():
    print("=" * 60)
    print("Etrid Wallet - Full Xcode Project Regenerator")
    print("=" * 60)

    # Paths
    script_dir = Path(__file__).parent
    output_dir = script_dir / "EtridWalletApp"
    existing_app_dir = output_dir / PROJECT_NAME

    # Check if existing app directory has Swift files
    if not existing_app_dir.exists():
        print(f"\n❌ Error: {existing_app_dir} does not exist")
        print("Please ensure the app directory exists with Swift files")
        return

    print(f"\n🔍 Discovering Swift files in {existing_app_dir}...")
    source_files = discover_swift_files(existing_app_dir)

    if not source_files:
        print(f"\n❌ Error: No Swift files found in {existing_app_dir}")
        return

    print(f"✓ Found {len(source_files)} Swift files:")
    for sf in source_files:
        print(f"  - {sf}")

    # Backup existing project file
    project_path = output_dir / f"{PROJECT_NAME}.xcodeproj"
    if project_path.exists():
        print(f"\n🗑️  Removing old project file...")
        shutil.rmtree(project_path)

    print(f"\n🔨 Regenerating Xcode project file with all Swift files...")
    project_path.mkdir(parents=True, exist_ok=True)
    create_project_pbxproj(project_path, existing_app_dir, source_files)
    ...
    print(f"Swift files included: {len(source_files)}")
    print(f"\nNext steps:")
    print(f"  1. Open project: open {output_dir}/{PROJECT_NAME}.xcodeproj")
    print(f"  2. Or build from CLI: cd {output_dir} && xcodebuild -scheme {PROJECT_NAME} -destination 'platform=iOS Simulator,name=iPhone 16 Pro'")
    print()
